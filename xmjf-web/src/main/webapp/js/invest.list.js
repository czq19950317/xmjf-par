        ...
        for(var i=0;i<list.length;i++){
           ...
           trs=trs+"</td>";
            ...
            var href=ctx+"/basItem/itemDetailPage?itemId="+tempData.id;
             if(status==1){
                 trs=trs+"<P><a href='"+href+"'><input valid type='button' value='即将开标' class='countdownButton' /></a></P>";
...
function initInvestScale() {
    ...
    $(".itemScale").each(function () {
        var investScale=$(this).attr("data-val");//获取投资进度
...
function initDjs() {
   $(".countdown").each(function () {
       var syTime=$(this).attr("data-time");//当前记录时间
